...
// print but dont if OOB
void printChar(int c, int r, char n){
  if (r < 0) return;
  if (r >= 20) return;
  if (c < 0) return;
  if (c >= 3) return;
...
void communicate(){
  ...
  if (currentCycleTime < commCycleListenDelay) {
    ...
    if(radio.available() > 0) {
      ...
      dist[0] = getEZReadNumDist(inPayload.sensorData);
      dist[1] = getEZReadNumDist(inPayload.data2);
      dist[2] = getEZReadNumDist(inPayload.data3);
      lux = inPayload.data4;
      deg = inPayload.data5;
      /*
